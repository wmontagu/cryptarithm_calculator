...
# Templates should always be found in the root 'templates' directory
TEMPLATES_DIR = BASE_DIR / 'templates'
STATIC_DIR = BASE_DIR / 'static'
print(f"TEMPLATES_DIR: {TEMPLATES_DIR}")

# If templates directory doesn't exist, create debug info
...
def handler(event, context):
    ...
    Handles GET / by calling the FastAPI root endpoint and returns HTML.
    Handles all exceptions and always returns a valid HTTP response.
    ...
    try:
        ...
        print(f"Vercel handler called with path: {path}, method: {http_method}")
        
        # Handle all routes, not just root
        if http_method == 'GET':
            # Simulate a FastAPI request
            req = Request(scope={
                'type': 'http',
                'method': 'GET',
                'path': path,
            ...
            # Route to appropriate endpoint based on path
            if path == '/' or path == '/index.html':
                response = asyncio.run(read_root(req))
            elif path == '/about':
                response = asyncio.run(about(req))
            elif path == '/examples':
                response = asyncio.run(get_examples(req))
            elif path == '/health':
                health_data = asyncio.run(health_check())
                from fastapi.responses import JSONResponse
                response = JSONResponse(content=health_data)
            elif path.startswith('/static/'):
                # Static files should be handled by Vercel routes
                return {
                    'statusCode': 404,
                    'headers': {'Content-Type': 'text/html'},
                    'body': '<h1>404 Static file not found</h1>'
                }
            else:
                return {
                    'statusCode': 404,
        ...
        elif http_method == 'POST' and path == '/solve':
            # Cannot handle POST requests properly in this simple handler
            # This would need more complex request body parsing
            return {
                'statusCode': 501,
                'headers': {'Content-Type': 'text/html'},
                'body': '<h1>POST requests not supported in serverless function</h1>'
            }
        else:
            return {
                'statusCode': 405,
        ...
        return {
            'statusCode': 500,
            'headers': {'Content-Type': 'text/html'},
            'body': f'<h1>500 Internal Server Error</h1><pre>{str(e)}</pre>'
        }
